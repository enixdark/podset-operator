...
import (
	...
	appv1alpha1 "github.com/enixdark/podset-operator/pkg/apis/app/v1alpha1"
...
func (r *ReconcilePodSet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	out2, _ := json.Marshal(lbls)
	reqLogger.Info("label: ", "test1", out2)

	// out1, _ := json.Marshal(podSet)
	reqLogger.Info("pod: ", "test2", lbls)

	existingPods := &corev1.PodList{}
	selects := labels.SelectorFromSet(lbls)
	...
	out, _ := json.Marshal(selects)
	reqLogger.Info("tessssss: ", "test3", string(out))

	clientList := client.ListOptions{
		Namespace:     request.Namespace,
		LabelSelector: labels.SelectorFromSet(lbls),
	}

	outx, _ := json.Marshal(clientList)
	reqLogger.Info("tessssss: ", "test3", string(outx))
	...
	for _, pod := range existingPods.Items {
		...
		out, _ := json.Marshal(pod.Status)
		reqLogger.Info(string(out))
